...
sim_data = function(n = 2000, sn = 35, sigmaU = 0.25, pv = 0.15, k = 1, beta = matrix(data = c(5, 1), ncol = 1)) {
...
run_sett_vary_dispersion = function(k, sn = 35, nrep = 50) {
  ...
  for (r in 1:nrep) {
    ...
    ### Save results
    res |>
      write.csv(paste0("vary_k", k * 10, "_seed", sim_seed, ".csv"),
                row.names = FALSE)
  }
}
