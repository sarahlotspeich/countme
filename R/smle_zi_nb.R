...
smle_zi_nb = function(analysis_formula, error_formula, data, no_se = TRUE, pert_scale = 1,
  ...
  Z = sub(".*\\|\\s*(.*)$", "\\1", as.character(as.formula(analysis_formula))[3]) ## covariates for zero-inflated model
  ## Outcome model -------------------------------------------------------------
  Y = as.character(analysis_formula)[2] ## outcome
  X_val = as.character(error_formula)[2] ## error-free covariate
  CX_val = sub("^(.*?)\\s*\\|.*", "\\1", as.character(as.formula(analysis_formula))[3]) ## covariates for outcome model
  C = setdiff(
    x = all.vars(as.formula(paste("~", CX_val))[[2]]),
    y = X_val
  ) ## additional error-free covariates in count model
  analysis_formula_only = as.formula(paste(Y, "~", paste(CX_val, collapse = "+"))) ## create formula for outcome model without zero-inflation
  zeroinfl_formula_only = as.formula(paste(Y, "~", paste(Z, collapse = "+"))) ## create formula for outcome model without zero-inflation
  ## Error model ---------------------------------------------------------------
  ...
  analysis_mat = model.matrix(object = analysis_formula_only,
                              data = data)
  ## Extract zero-inflated model matrix (complete cases) for structure ---------
  zeroinfl_mat = model.matrix(object = zeroinfl_formula_only,
                              data = data)
  ## Re-extract covariate names ------------------------------------------------
  C = setdiff(colnames(analysis_mat)[-1], X_val)
  Z = colnames(zeroinfl_mat)[-1]
  ...
  x_obs = sort(unique(data[1:n, X_val]))
  m = length(x_obs)
  ## Save static (X*,X,Y,C,Z) since they don't change --------------------------
  mm_val = model.matrix(object = analysis_formula_only, data = data[c(1:n), ])
  colnames(mm_val)[colnames(mm_val) == "(Intercept)"] = "int"
  mm_val_zi = model.matrix(object = zeroinfl_formula_only, data = data[c(1:n), ])
  colnames(mm_val_zi)[colnames(mm_val_zi) == "(Intercept)"] = "int"
  comp_dat_val = data.frame(mm_val,
                            mm_val_zi[, setdiff(colnames(mm_val_zi), colnames(mm_val)), drop = FALSE],
  ...
  mm_unval = model.matrix(object = analysis_formula_only, data = unval_rep)
  colnames(mm_unval)[colnames(mm_unval) == "(Intercept)"] = "int"
  mm_unval[, X_val] = rep(x_obs, each = (N - n))  # substitute x_obs values
  mm_unval_zi = model.matrix(object = zeroinfl_formula_only, data = unval_rep)
  colnames(mm_unval_zi)[colnames(mm_unval_zi) == "(Intercept)"] = "int"
  comp_dat_unval = data.frame(mm_unval,
                              mm_unval_zi[, setdiff(colnames(mm_unval_zi), colnames(mm_unval)), drop = FALSE],
  ...
  eta_cols = c("int", Z)
  prev_beta_fit = beta_fit0 = suppressWarnings(
    zic.reg(
  ...
  prev_beta = beta0 = matrix(data = beta_fit0$coef[grepl(pattern = "ct_", x = names(beta_fit0$coef))],
                             ncol = 1)
  prev_eta = eta0 = matrix(data = beta_fit0$coef[grepl(pattern = "zi_", x = names(beta_fit0$coef))],
                           ncol = 1)
  prev_theta = theta0 = beta_fit0$coef["Theta"]
  ...
  while(it <= max_iter & !CONVERGED) {
    ############################################################################
    # E Step -------------------------------------------------------------------
    E_step_res = E_step_zeroinfl_nb(prev_beta_fit = prev_beta_fit,
                                    beta_cols = beta_cols,
                                    eta_cols = eta_cols,
                                    Y = Y,
                                    prev_p = prev_p,
    ...
    it = it + 1
    prev_beta_fit = M_step_res$new_beta_fit
    prev_beta = M_step_res$new_beta
    prev_eta = M_step_res$new_eta
    prev_theta = M_step_res$new_theta
    prev_p = M_step_res$new_p
  }
  ### Name rows of coefficients before preparing to return (below)
  new_coeff = c(M_step_res$new_beta,
                M_step_res$new_eta,
                M_step_res$new_theta)
  names(new_coeff) = c(paste0("count:", beta_cols),
                       paste0("zero:", eta_cols),
                       "dispersion")
  ...
  if(no_se){
    ...
    if(output == "coeff") {
      ...
      od_loglik_conv = smle_zi_nb_od_ll(
        N = N,
        n = n,
        prev_beta_fit = M_step_res$new_beta_fit,
        Y = Y,
    ...
    single_pert_coeff = sapply(X = seq(1, ncol(I_coeff)),
                               FUN = smle_zi_nb_prof_ll,
    ...
    if (any(is.na(single_pert_coeff))) {
      ...
      spt_wide = matrix(data = rep(c(single_pert_coeff),
                        ...
                        ncol = ncol(I_coeff),
                        byrow = FALSE)
      #for the each kth row of single_pert_coeff add to the kth row / kth column of I_coeff
    ...
    rownames(coeff_df) = c(paste0("count:", beta_cols),
                           paste0("zero:", eta_cols),
                           "dispersion")
    colnames(coeff_df) = c("Estimate", "Std. Error", "z value", "Pr(>|z|)")
    rownames(cov_coeff) = colnames(cov_coeff) = c(paste0("count:", beta_cols),
                                                  paste0("zero:", eta_cols),
                                                  "dispersion")
    if(output == "coeff") {
      return(list(coefficients = coeff_df,
